...
#20210820: increased r0['G'] from 3.8 to 4.2 to prevent steric clashes
#20210820B: restored previous r0['G']; mass of CAHA2 shared between the two a-protons of G
...
with open("data.CG", "w") as tf:
    ...
    for n, el in enumerate(set(sequence)):
        # first we write all CAs, which actually have the mass of CAHA (except for Gly)
        if el.decode()=="GLY":
            tf.write(str(n+1)+" "+str(12.011/2. + 1.0079)+"\n")
        else:
            tf.write(str(n+1)+" "+str(12.011 + 1.0079)+"\n")
...
#     tf.write("read_data data.CG add append\n")
    ...
    tf.write("\n")
    tf.write("dump 1 all xtc 250 traj.xtc\n")
